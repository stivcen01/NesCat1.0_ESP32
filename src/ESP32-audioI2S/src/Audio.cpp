...
Audio::Audio() {
    ...
    m_i2s_config.sample_rate          = 160000;
    m_i2s_config.bits_per_sample      = I2S_BITS_PER_SAMPLE_16BIT;
    m_i2s_config.channel_format       = I2S_CHANNEL_FMT_RIGHT_LEFT;
///    m_i2s_config.communication_format = (i2s_comm_format_t)( I2S_COMM_FORMAT_I2S | I2S_COMM_FORMAT_I2S_MSB);
// I2S_COMM_FORMAT_PCM = for NES
    m_i2s_config.communication_format = (i2s_comm_format_t)(I2S_COMM_FORMAT_PCM | I2S_COMM_FORMAT_I2S | I2S_COMM_FORMAT_I2S_MSB);
...
esp_err_t Audio::I2Sstop(uint8_t i2s_num) {
	///free(m_buffer);
	///free(m_outBuff); //NCX: here
	///MP3Decoder_FreeBuffers();
	
    return i2s_stop((i2s_port_t) i2s_num);
}

void Audio::Uninstall() {
...
///	bool Audio::connecttoFS(SdFatSoftSpi<SOFTSD_MISO_PIN, SOFTSD_MOSI_PIN, SOFTSD_SCK_PIN> &fs, String file){
bool Audio::connecttoFS(SdFat &fs, String file){
...
//-------------------------------------------------------------------------------
    String afn;  //audioFileName
    char audiofilename[256];
    audiofile.getName(audiofilename,256);
    afn =  String(audiofilename);
//-------------------------------------------------------------------------------	
...
void Audio::stopSong(){
    ...
    memset(m_outBuff, 0, 2*2048);     //NCX fix: Clear OutputBuffer
...
//*******************************************************************************
 int16_t         n_Sample[16];
 n_Sample[0]=Gain(m_outBuff[m_curSample * 2]);
 n_Sample[1]=Gain(m_outBuff[m_curSample * 2+1]);
 n_Sample[2]=Gain(m_outBuff[m_curSample * 2+2]);
 n_Sample[3]=Gain(m_outBuff[m_curSample * 2+3]);
 n_Sample[4]=Gain(m_outBuff[m_curSample * 2+4]);
 n_Sample[5]=Gain(m_outBuff[m_curSample * 2+5]);
 n_Sample[6]=Gain(m_outBuff[m_curSample * 2+6]);
 n_Sample[7]=Gain(m_outBuff[m_curSample * 2+7]);
 n_Sample[8]=Gain(m_outBuff[m_curSample * 2+8]);
 n_Sample[9]=Gain(m_outBuff[m_curSample * 2+9]);
 n_Sample[10]=Gain(m_outBuff[m_curSample * 2+10]);
 n_Sample[11]=Gain(m_outBuff[m_curSample * 2+11]);
 n_Sample[12]=Gain(m_outBuff[m_curSample * 2+12]);
 n_Sample[13]=Gain(m_outBuff[m_curSample * 2+13]);
 n_Sample[14]=Gain(m_outBuff[m_curSample * 2+14]);
 n_Sample[15]=Gain(m_outBuff[m_curSample * 2+15]);
//*******************************************************************************
	               i2s_write(I2S_NUM_1, (const char *)&n_Sample, 2 * 16, &m_i2s_bytesWritten, 1000);
//*******************************************************************************
